...
export default function VerifyPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (data.success && data.accessToken) {
        // use auth hook to save token
        await auth.onLogin(data.accessToken, data.user);
        // If userExists === false, redirect to register page; else go to home
  ...
  return (
    <div style={{ maxWidth: 420, margin: "80px auto", padding: 20 }}>
      ...
      <button onClick={submit} disabled={loading} style={{ marginTop: 16, width: "100%", padding: 10 }}>
        {loading ? "Verifying..." : "Verify OTP"}
      </button>
      <button onClick={resend} style={{ marginTop: 8, width: "100%", padding: 8 }}>
        Resend OTP
      </button>
    </div>
  );
